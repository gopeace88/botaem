...
const playbookSchema = {
  ...
  properties: {
    metadata: {
      type: 'object',
      required: ['id', 'name', 'version', 'category', 'difficulty'],
      properties: {
        ...
        category: {
          type: 'string',
          enum: ['회원관리', '사업선정', '교부관리', '집행관리', '정산관리', '사후관리', '사용자지원', '테스트', '기타'],
        },
        difficulty: {
          type: 'string',
          enum: ['쉬움', '보통', '어려움'],
  ...
  $defs: {
    step: {
      ...
      properties: {
        ...
        value: { type: 'string' },
        message: { type: 'string' },
        wait_for: { type: 'string', enum: ['element', 'navigation', 'network', 'user', 'user_input'] },
