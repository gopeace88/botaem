...
export class PlaybookParser {
  ...
  ): Record<string, Playbook['variables'] extends Record<string, infer V> ? V : never> {
    const variables: Record<string, Playbook['variables'] extends Record<string, infer V> ? V : never> = {};

    for (const [key, value] of Object.entries(variablesData)) {
      const varDef = value as Record<string, unknown>;
      variables[key] = {
        type: varDef.type as 'string' | 'number' | 'date' | 'select' | 'boolean',
        ...
        options: varDef.options as string[] | Array<{ value: string; label: string }> | undefined,
        validation: varDef.validation as {
          min?: number;
          max?: number;
          minLength?: number;
          maxLength?: number;
